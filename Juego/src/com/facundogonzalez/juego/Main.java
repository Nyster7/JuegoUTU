...
public class Main {
	...
	public static void main(String[] args) {
		...
		enemigosPlaya[1] = new Enemigo("Bandido veterano", 100, 150, 50, 20, 10, 40, 7, 50, "Pu�alada precisa");
		...
		// Set elite
		partesEquipo [6] = new Equipo("Casco M12 de �lite", "Cabeza", "�lite", 0, 0, 10, 0, 0);
		partesEquipo [7] = new Equipo("Arnadura Troski de �lite", "Torso", "�lite", 0, 10, 15, -5, 0);
		partesEquipo [8] = new Equipo("Pantalones Galife de �lite", "Piernas", "�lite", 0, 10, 5, 0, 0);
		partesEquipo [9] = new Equipo("Botas Stalin de �lite", "Pies", "�lite", 0, 0, 5, 0, 0);
		partesEquipo [10] = new Equipo("Lanza de Iv�n el Terrible", "ManoIzquierda", "�lite", 15, 0, 0, 0, 0);
		partesEquipo [11] = new Equipo("Gran Escudo del Le�n Rojo", "ManoDerecha", "�lite", 0, 5, 10, -10, 0);
		...
		while(confirmarEdad == false) {
			...
			int respuesta = Logica.eleccionJugador("Tienes " + edadJugador + " a�os. \nEs correcto? \n1. Si \n2. No" , 2);
			if (respuesta == 1) {
				confirmarEdad = true;
				break;
			}
		...
		while(juegoCorriendo) {
				...
				if (respuesta == 1) {
					if(eventoInicio == false) {
						System.out.println("Intentas abrir el cofre pero parece que este no abrira sin la llave."
								...
								+ "\nTe causa 30 de da�o, preparate para luchar!");
			...
			while(vidaEnemigo > 0) {
				...
				if(respuestaUsuario.equals("1")) {
					...
					System.out.println("\t> Atacaste a " + enemigoGenerado.getNombre() + ", haciendole " + ataqueRealizado + " de da�o." );
					if(vidaEnemigo >0 ) { if(ataqueRecibido > 50) {System.out.println("\t> " + enemigoGenerado.getNombre() + " uso " + enemigoGenerado.ataquePotente + ".");}}
					if(vidaEnemigo >0 ) {System.out.println("\t> Recibiste " + ataqueRecibido + " de da�o.");}
					
					
					//Si el enemigo te mata
					if (jugador.getSalud() <1) {
						Logica.imprimirSeparador(50);
						System.out.println("\t> Recibiste mucho da�o, te sientes demasiado debil para continuar.");
						Logica.ingresaAlgo();
						break JUEGO;
					}
				...
				//Opcion de tomar pocion	
					...
					if(numeroPociones > 0) {
						...
						System.out.println("\t> Tomas una pocion que te cura " + curacionPocion + " de vida."
										+ "\n\t> Ahora estas con " +  jugador.getSalud() + " de vida, podes seguir a las pi�as."
				...
				//Opcion de escapar la pelea
					...
					if(aleatorio.nextInt(50) > exitoEscape) {
						...
						Logica.imprimirSeparador(50);
						System.out.println("\t> Recibiste " + ataqueRecibido + " de da�o.");
						if (jugador.getSalud() <1) {
							Logica.imprimirSeparador(50);
							System.out.println("\t> Recibiste mucho da�o, te sientes demasiado debil para continuar.");
							oroAcumulado = 0;
				...
				if (respuestaUsuario.equals("1")) {
					...
					if (equipoDropeado.getNombre().equals(jugador.cabeza.getNombre()) 
						...
						if(equipoDropeado.getSet().equals("Turing")) {
							setTuring --;
							setTuring ++;
						}else if(equipoDropeado.getSet().equals("�lite")) {
							setElite --;
							setElite ++;
						}
					}
		...
		if(enemigosVencidos == 8) {
			Logica.imprimirEncabezado("Nivel de aventurero: H�roe Supremo");
